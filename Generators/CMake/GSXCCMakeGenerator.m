...
- (void) createInfoPlist: (NSString *)projName;
@end
...
- (NSString *) cmakePreamble: (NSString *)projName
{
  NSString *output = @"";
...
- (NSString *) cmakeResourceFiles: (NSArray *)array
		      projectType: (NSString *)projType
			     name: (NSString *)projName
{
  NSString *output = @"";
  NSString *file = nil;

  if ([projType isEqualToString: @"tool"] || array == nil || [array count] == 0)
    {
      return @"";
    }
  ...
  NSString *infoPlistName = [NSString stringWithFormat: @"%@Info.plist", projName];
...
- (NSString *) cmakeDeclareProjectType: (NSString *)projType
				  name: (NSString *)projName
{
...
- (NSString *) cmakeIncludeDirsName: (NSString *)projName
...
- (NSString *) cmakeTargetCompileOptions: (NSString *)projName
{
  NSString *output = [NSString stringWithFormat: @"# Compile options\n"
			       @"target_compile_options(%@ PRIVATE ", projName];
...
- (NSString *) cmakeTargetLinkOptions: (NSString *)projName
			  projectType: (NSString *)projType
{
  NSString *output = [NSString stringWithFormat: @"# Link options\n"
			       @"target_link_options(%@ PRIVATE ", projName];
...
- (NSString *) cmakeDeclareTarget: (NSString *)projName
{
  NSString *output = @"";
...
- (void) createInfoPlist: (NSString *)projName
{
  NSString *inputFileName = [NSString stringWithFormat: @"%@Info.plist", projName];
...
- (BOOL) generate
{
  ...
  // Declare target
  outputString = [outputString stringByAppendingString:
			      [self cmakeDeclareTarget: projName]];
  ...
  // Include dirs
  outputString = [outputString stringByAppendingString:
			    [self cmakeIncludeDirsName: projName
					includeDirList: nil]];
  ...
  // Compile options...
  outputString = [outputString stringByAppendingString:
		       [self cmakeTargetCompileOptions: projName]];

  // Link options...
  outputString = [outputString stringByAppendingString:
			  [self cmakeTargetLinkOptions: projName projectType: projectType]];
  
  // Handle project type... this builds the directory structure needed for a given type.
  outputString = [outputString stringByAppendingString:
			 [self cmakeDeclareProjectType: projectType
						  name: projName]];
